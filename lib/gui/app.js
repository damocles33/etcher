...
const information = require('../shared/information');
...
app.run((AnalyticsService, ErrorService, UpdateNotifierService, SelectionStateModel) => {
  ...
  const currentVersion = information.getCurrentVersion();
  const releaseType = information.getReleaseType(currentVersion);

  if (_.some([
    !shouldCheckForUpdates,
    process.env.ETCHER_DISABLE_UPDATES,
    releaseType === information.RELEASE_TYPE.UNKNOWN
  ])) {
  ...
  const bucketUrl = s3Packages.getBucketUrlFromReleaseType(releaseType);
  ...
  s3Packages.getLatestVersion(bucketUrl).then((latestVersion) => {
