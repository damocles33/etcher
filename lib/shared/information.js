 ...
 * const releaseType = information.getReleaseType(version);
 *
 * if (releaseType === information.RELEASE_TYPE.PRODUCTION) {
 *   console.log('This is a production release!');
 * }
 */
exports.getReleaseType = (version) => {
  const GIT_HASH_REGEX = /^[0-9a-f]{7,40}$/;
  const buildNumber = semver.parse(version).build;
 ...
 * @public
 *
 * @returns {String} version - the current version
 *
 * @example
 * const currentVersion = information.getCurrentVersion();
 * console.log(currentVersion);
 */
exports.getCurrentVersion = _.constant(packageJSON.version);
